...
const IdCode = window.location.href.replace(/ht.+de=/, "").replace("#", "").replace(/\&fbclid.+/, "").replace("&m=1", "").replaceAll('%EF%BF%BD', '').toUpperCase();
	} catch (error) {if (error instanceof TypeError && error.message.includes("replaceAll is not a function")) {alert("Browser tidak mendukung penampilan, Silahkan buka dari chrome.");}}	
